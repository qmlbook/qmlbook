...
Currently, we will focus on the fundamental visual elements, such as ``Item``, ``Rectangle``, ``Text``, ``Image`` and ``MouseArea``. However, by using the Qt Quick Controls 2.0 module, it is possible to create user interfaces built from standard platform components such as buttons, labels and sliders. By using the Quick Controls module, your user interface will automatically adapt to the platform it is running on and look like any other native application.
