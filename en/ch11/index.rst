...
Now our file should be reachable via ``http://localhost:8000/main.qml``. You can test it with::

    $ curl http://localhost:8000/main.qml

Or just point your browser to the location. Your browser does not understand QML and will not be able to render the document through. We need to create now such a browser for QML documents. To render the document we need to point our ``qmlscene`` to the location. Unfortunately the ``qmlscene`` is limited to local files only. We could overcome this limitation by writing our own ``qmlscene`` replacement or simple dynamically load it using QML. We choose the dynamic loading as it work just fine. For this we use a loader element to retrieve for us the remote document.
...
We modify our main.qml to use the button now::
